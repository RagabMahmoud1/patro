...
class stock_summary(models.Model):
    ...
    total_balance = fields.Float(string="Total Balance", compute="_calc_total_balance",store=False)
    ...
    @api.depends('line_ids.qty_balance')
        ...
        for rec in self:
            ...
            for line in rec.line_ids:
                if line.qty_balance>0:
                    print("line.qty_balance",line.qty_balance)
                    sum+=line.qty_balance
    ...
    def process_lines_nosn(self, line_type, date):
        ...
        if line_type=="beg":
            for beg in res:
                ...
                qty,product_uom_id = self.convert_uom_qty(product_id, sm_uom_id, qty )
                product = self.env['product.product'].browse(product_id)
                if self.ore_id and self.ore_id.id != product.ore_id.id:
                    sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue
                if self.session_id and self.session_id.id != product.session_id.id:
                    sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue
                if self.category_id and self.category_id.id != product.categ_id.id:
                    sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue
                if self.model_year and self.model_year != product.model_year:
                    sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue

                if self.color and self.color != product.color:
                    sql2 = "delete from vit_stock_summary_lin ewhere stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue
                if self.size and self.size != product.size:
                    sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
                     self.id, product_id)
                    cr.execute(sql2)
                    continue
            ...
            for incoming in res:
                ...
                if not stock_summary_line.search([('stock_summary_id','=',self.id),('product_id','=',product_id)]):
                    product=self.env['product.product'].browse(product_id)
                    if self.ore_id and self.ore_id.id!=product.ore_id.id:
                        sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % ( self.id, product_id)
                        cr.execute(sql2)
                        continue
                    if self.session_id and self.session_id.id != product.session_id.id:
                        sql2 = "delete from vit_stock_summary_line where stock_summary_id = %s and product_id=%s" % (
    ...
    def export(self):
        ...
        for record in self:
            ...
            worksheet.write(0, 2, record.location_id.display_name)
            worksheet.write(0, 3, "Category:")
            worksheet.write(0, 4, record.category_id.display_name)
            worksheet.write(0, 5, "Date Start:")
            worksheet.write(0, 6, str(record.date_start))
            worksheet.write(0, 7, "Date End	:")
            worksheet.write(0, 8, str(record.date_end))
            worksheet.write(0, 9, "Color:")
            worksheet.write(0, 10, str(record.color))

            worksheet.write(0, 11, "Size:")
            worksheet.write(0, 12, str(record.size))
            worksheet.write(0, 13, "Model Year:")

            worksheet.write(0, 14, str(record.model_year))

            worksheet.write(0, 15, "نوع الخامة")
            worksheet.write(0, 16, str(record.ore_id.name))

            worksheet.write(0, 17, "الموسم")
            worksheet.write(0, 18, str(record.session_id.name))

            final_data.append(header_name)
            for z in record.line_ids:
                line_data = [
                             str(z.product_id.name),
                             str(z.lot_id.name),
                             str(z.name),
                             z.qty_start,
                             z.qty_in,
