...
class ProductProduct(models.Model):
    ...
    barcode_value = fields.Char(compute='_compute_barcode_values', store=True)

    @api.depends('product_template_attribute_value_ids')
    def _compute_barcode_values(self):
        for product in self:
            value = ''
            ...
            # value += str(product.list_price) + ' '
            # if product.currency_id:
            #     value += str(product.currency_id.symbol)
            # if product.default_code:
            #     value += str(product.default_code)
            # if product.barcode:
            #     value += str(product.barcode)
            print(value,8888888888888888888)
