odoo.define('project_custom.sale', function (require) {
    "use strict";

    const models = require('point_of_sale.models');
    var SaleOrderManagementScreen = require('pos_sale.SaleOrderManagementScreen');
            ...
            const {confirmed, payload: selectedOption} = await this.showPopup('SelectionPopup',
                {
                    ...
                    list: [{id: "0", label: "Apply a down payment", item: false}, {
                        id: "1",
                        label: "Settle the order",
                        item: true
                    }],
                });
            ...
            if (confirmed) {
                ...
                if (selectedOption) {
                    ...
                    const {confirmed, payload} = await this.showPopup('NumberPopup', {
                        title: sprintf(this.env._t("Fixed Amount of %s"), this.env.pos.format_currency(sale_order.amount_total)),
                        startingValue: 0,
                    });
                    if (confirmed) {
